  ...
  class DOMRedactor {
    ...
    redactPageDOM(rootNode = document.body, isPartial = false) {
      ...
      for (const textNode of textNodes) {
        ...
        // Contextual DOM detection for Passport Numbers in UI cards, labels, and forms
        const trimmedVal = rawText.trim();
        if (
          trimmedVal &&
          trimmedVal.length >= 6 &&
          trimmedVal.length <= 12 &&
          /^[A-Z0-9]+$/i.test(trimmedVal) &&
          !/^\[[A-Z0-9_]+\]$/i.test(trimmedVal) &&
          !/^(?:passport|republic|specimen|nationality|country|surname|signature|number|active|pending)$/i.test(trimmedVal)
        ) {
          const passportLabelRegex = /(?:passport\s*(?:no\.?|num\.?|number|#)?|passeport\s*(?:no\.?|num\.?|#)?|paspò\s*(?:no\.?|num\.?|#)?|doc(?:ument)?\s*(?:no\.?|num\.?|number|#)|id\s*(?:no|num|number))/i;
          let isPassport = false;

          // Check previous element sibling (e.g. <span class="field-label">PASSPORT NO</span>)
          if (parent && parent.previousElementSibling && passportLabelRegex.test(parent.previousElementSibling.textContent || '')) {
            isPassport = true;
          }

          // Check previous sibling text nodes or elements inside the same parent
          if (!isPassport && textNode.previousSibling) {
            let sib = textNode.previousSibling;
            while (sib) {
              if (passportLabelRegex.test(sib.textContent || '')) {
                isPassport = true;
                break;
              }
          ...
          // Check parent's label element or enclosing field container
          if (!isPassport && parent) {
            const container = (typeof parent.closest === 'function') 
              ? (parent.closest('.field-group, .field, [class*="passport"], [id*="passport"]') || parent.parentElement)
              : parent.parentElement;
            if (container) {
              const labelEl = container.querySelector ? container.querySelector('.field-label, label, dt, .label, th') : null;
              if (labelEl && labelEl !== parent && passportLabelRegex.test(labelEl.textContent || '')) {
                isPassport = true;
              }
              const containerAttrs = `${container.className || ''} ${container.id || ''} ${container.getAttribute?.('data-field') || ''}`;
              if (/passport[-_]?(?:no|num|number)?|doc[-_]?no/i.test(containerAttrs)) {
                isPassport = true;
              }
            }
          }
          ...
          if (isPassport) {
            const sIdx = rawText.indexOf(trimmedVal);
            if (sIdx !== -1) {
              extraEntities.push({
                text: trimmedVal,
                label: 'PASSPORT_NO',
                start: sIdx,
                end: sIdx + trimmedVal.length
              });
            }
          }
        }
    ...
    redactDOMFaces(faceBoxes = []) {
      ...
      for (const face of faceBoxes) {
        ...
        const left = Math.round(face.x + scrollX);
        const top = Math.round(face.y + scrollY);
        const w = Math.round(face.width);
        const h = Math.round(face.height);

        // Deduplication: prevent duplicate stacking of face overlays
        const existing = (typeof document !== 'undefined') ? Array.from(document.querySelectorAll('.ps-face-overlay')).some(el => {
          const elLeft = parseInt(el.style.left, 10);
          const elTop = parseInt(el.style.top, 10);
          return Math.abs(elLeft - left) < 15 && Math.abs(elTop - top) < 15;
        }) : false;
    ...
    redactDOMOCRBoxes(ocrBoxes = []) {
      ...
      // Collect existing face overlay bounds to prevent duplicate / nested overlays over faces
      const faceOverlays = (typeof document !== 'undefined') ? Array.from(document.querySelectorAll('.ps-face-overlay')) : [];
      const faceRects = faceOverlays.map(el => {
      ...
      for (const box of ocrBoxes) {
        ...
        const left = Math.round(box.x + (typeof window !== 'undefined' ? window.scrollX : 0));
        const top = Math.round(box.y + (typeof window !== 'undefined' ? window.scrollY : 0));
        const w = Math.round(box.width);
        const h = Math.round(box.height);
        const right = left + w;
        const bottom = top + h;

        // Skip OCR overlays that overlap with an already redacted face overlay
        const isInsideFace = faceRects.some(fr => 
          (left < fr.right && right > fr.left && top < fr.bottom && bottom > fr.top)
        );
        ...
        overlay.style.cssText = `
          ...
          border: 1.5px solid rgba(217, 119, 87, 0.85) !important;
