...
(function() {
  ...
  class ActionValidator {
    ...
    validateSingleAction(rawAction) {
      ...
      let targetText = rawAction.targetText || null;
      if (!selector && rawAction.target) {
        if (typeof rawAction.target === 'string') {
          ...
          if (rawAction.target.type === 'selector' || rawAction.target.type === 'css') {
            ...
            targetText = rawAction.target.value;
            const safeEscaped = rawAction.target.value.replace(/"/g, '\\"');
            selector = `[value="${safeEscaped}"], [aria-label="${safeEscaped}"]`;
          } else {
            selector = rawAction.target.value;
          }
        }
      ...
      if (actionName === 'click' && (lowerSel.includes('submit') || lowerText.includes('submit'))) {
        return {
          valid: true,
