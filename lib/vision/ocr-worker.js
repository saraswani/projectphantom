  ...
  class OCRWorker {
    ...
    async init() {
      ...
      try {
        ...
        const workerOptions = {
          workerPath: extUrl ? (extUrl + 'lib/vision/tesseract/worker.min.js') : undefined,
          corePath: extUrl ? (extUrl + 'lib/vision/tesseract/tesseract-core.wasm.js') : undefined,
          langPath: extUrl ? (extUrl + 'lib/vision/tesseract/') : undefined,
          workerBlobURL: false,
          gzip: true
        };

        try {
          this.worker = await tesseractLib.createWorker('eng', 1, workerOptions);
        } catch (blobErr) {
          this.worker = await tesseractLib.createWorker('eng');
        }
