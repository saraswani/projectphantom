...
(function() {
  ...
  class LocalFaceDetector {
    ...
    detectHeuristicFaces(canvas, ctx) {
      ...
      const isPortraitAspect = aspect >= 0.65 && aspect <= 1.85;

      let imgData;
      try {
        imgData = ctx.getImageData(0, 0, width, height);
      } catch (_) {
        // When canvas is tainted by cross-origin data, direct pixel read is blocked by browser security.
        // Fall back to biometric portrait face box so cross-origin faces are NEVER left unredacted!
        if (isPortraitAspect && width >= 28 && height >= 28) {
          return [{
      ...
      // Default biometric portrait face crop for avatar/headshot/passport images
      if (isPortraitAspect && width >= 28 && height >= 28) {
        return [{
          x: Math.round(width * 0.12),
          y: Math.round(height * 0.08),
    ...
    async detectFacesInElement(element) {
      ...
      try {
        ...
        // Failsafe 2: If faceBoxes is STILL empty (e.g. tainted canvas or subtle face angles),
        // and element is a portrait/square photo (ID, passport, headshot, avatar), REDACT IT!
        if (faceBoxes.length === 0 && nw >= 28 && nh >= 28) {
          const aspect = nh / (nw || 1);
          const isPortrait = aspect >= 0.65 && aspect <= 1.85;
          const isPhotoNamed = (element.className || element.id || element.alt || element.src || '').toString().match(/photo|portrait|avatar|face|user|profile|person|passport|headshot/i);
          if (isPortrait || isPhotoNamed) {
            faceBoxes.push({
              x: Math.round(nw * 0.12),
              y: Math.round(nh * 0.08),
              width: Math.round(nw * 0.76),
              height: Math.round(nh * 0.72),
              confidence: 0.85,
              isHeuristic: true
            });
          }
    ...
    async scanSingleImage(img) {
      if (!img || (img.closest && img.closest('#privacyshield-root'))) return [];

      // If container element, scan inner img if present
      if (img.tagName && img.tagName.toLowerCase() !== 'img') {
        const innerImg = img.querySelector && img.querySelector('img');
        if (innerImg) return await this.scanSingleImage(innerImg);
      }
      ...
      if (img.tagName && img.tagName.toLowerCase() === 'img') {
        if (!img.complete && img.naturalWidth === 0 && !img.src) return [];
      }

      // Attempt CORS-anonymous reload to allow canvas pixel access (600ms gives time for 500KB-1MB images)
      let targetImg = img;
      ...
      const nw = fit.naturalWidth || img.naturalWidth || rect.width || 100;
      const nh = fit.naturalHeight || img.naturalHeight || rect.height || 100;
    ...
    async scanPageImages() {
      ...
      const allFaceBoxes = [];
      const rawElements = Array.from(document.querySelectorAll('img, picture img, [role="img"], .photo-box, .avatar, [class*="avatar"], [class*="photo"], [class*="headshot"], [class*="portrait"]'));
      const elements = [];
      const seenNodes = new Set();

      for (const el of rawElements) {
        if (!el || seenNodes.has(el)) continue;
        if (el.tagName && el.tagName.toLowerCase() === 'img') {
          seenNodes.add(el);
          elements.push(el);
        } else {
          const innerImg = el.querySelector && el.querySelector('img');
          if (innerImg && !seenNodes.has(innerImg)) {
            seenNodes.add(innerImg);
            elements.push(innerImg);
          } else if (!innerImg) {
            seenNodes.add(el);
            elements.push(el);
          }
      ...
      for (const img of elements) {
        ...
        const rect = img.getBoundingClientRect ? img.getBoundingClientRect() : { width: 100, height: 100, top: 0, bottom: 100, left: 0, right: 100 };
        if (rect.width < 28 || rect.height < 28) { skippedTooSmall++; continue; }
        if (img.tagName && img.tagName.toLowerCase() === 'img' && !img.complete && img.naturalWidth === 0 && !img.src) {
          skippedNotLoaded++;
          continue;
        }
      ...
      // Prioritize top 16 most prominent images in viewport to ensure immediate response
      const prioritized = candidates.slice(0, 16);
